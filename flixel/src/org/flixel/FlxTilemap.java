...
public class FlxTilemap extends FlxObject
{
	 ...
	 */
	protected FlxArray<FlxTile> _tileObjects;	
	/**
	...
	public FlxTilemap()
	{
		...
		heightInTiles = 0;
		totalTiles = 0;
		_buffers = new FlxArray<FlxTilemapBuffer>();
	...
	public void destroy()
	{
		...
		while(i < l)
			((FlxTile)_tileObjects.get(i++)).destroy();
		_tileObjects = null;
		i = 0;
		l = _buffers.size;
		while(i < l)
	...
	public FlxTilemap loadMap(String MapData, TextureRegion TileGraphic, int TileWidth, int TileHeight, int AutoTile, int StartingIndex, int DrawIndex, int CollideIndex)
	{
		...
		while(row < heightInTiles)
		{
			...
			while(column < widthInTiles)
				_data.add(Integer.parseInt(columns[column++]));
		}
		...
		if(auto > OFF)
			l++;
		_tileObjects = new FlxArray<FlxTile>();
...
//		buffer.draw();//TODO: needed?
	...
	public FlxArray<Integer> getData(boolean Simple)
	{
		if(!Simple)
			return _data;
		...
		int i = 0;
		int l = _data.size;
		FlxArray<Integer> data = new FlxArray<Integer>(); 
		while(i < l)
		{			
	...
	public boolean overlapsWithCallback(FlxObject Object, AFlxObject Callback, boolean FlipCallbackParams, FlxPoint Position)
	{
		...
		int selectionWidth = (int) (selectionX + (FlxU.ceil(Object.width/_tileWidth)) + 1);
		int selectionHeight = (int) (selectionY + FlxU.ceil(Object.height/_tileHeight) + 1);
				...
				if(tile.allowCollisions > 0)
				{
					...
					if(Callback != null)
					{
						if(FlipCallbackParams)
							overlapFound = Callback.overlapsWith(Object,tile);
						else
							overlapFound = Callback.overlapsWith(tile,Object);
					}
					else
	...
	{
			if(!InScreenSpace)
				return _tileObjects.get((int) (_data.get((int) ((Point.y-y)/_tileHeight))*widthInTiles + (Point.x-x)/_tileWidth)).allowCollisions > 0;
		...
		return _tileObjects.get((int) (_data.get((int) ((Point.y-_point.y)/_tileHeight))*widthInTiles + (Point.x-_point.x)/_tileWidth)).allowCollisions > 0;
	}
	...
	public int[] getTileInstances(int Index)
	{
		int[] array = null;
		int i = 0;
	 ...
	 */
	public FlxArray<FlxPoint> getTileCoords(int Index, boolean Midpoint)
	{
		FlxArray<FlxPoint> array = null;
	...
	public FlxArray<FlxPoint> getTileCoords(int Index)
	{
	 ...
	 */
	public void setTileProperties(int Tile, int AllowCollisions, AFlxTile Callback, int Range)
	{
		if(Range <= 0)
			Range = 1;
		FlxTile tile;
	...
	{
		setTileProperties(Tile, AllowCollisions, Callback, 1);
	}
	...
	{
		setTileProperties(Tile, AllowCollisions, null, 1);
	}
	/**
	 * Adjust collision settings and/or bind a callback function to a range of tiles.
	 * This callback function, if present, is triggered by calls to overlap() or overlapsWithCallback().
	...
	{
		setTileProperties(Tile, 0x1111, null, 1);
	}
