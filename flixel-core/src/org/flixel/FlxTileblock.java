...
/**
 * This is a basic "environment object" class, used to create simple walls and
 * floors. It can be filled with a random selection of tiles to quickly add
 * detail.
 * 
 * @author Ka Wing Chin
...
{
	/**
	 * Creates a new <code>FlxBlock</code> object with the specified position
	 * and size.
	 * 
	...
	public FlxTileblock loadTiles(String TileGraphic, int TileWidth, int TileHeight, int Empties)
	{
		...
		// Then prep the "canvas" as it were (just doublechecking that the size
		...
		while(row < heightInTiles)
		{
			...
			while(column < widthInTiles)
			{
				if(FlxG.random() * total > Empties)
				{
					sprite.randomFrame();
					sprite.drawFrame();
					stamp(brushPixmap, sprite.framePixels.getRegionX(), sprite.framePixels.getRegionY() - sprite.frameHeight, sprite.frameWidth, sprite.frameHeight, destinationX
							+ _pixels.getRegionX(), destinationY + _pixels.getRegionY());
				}
