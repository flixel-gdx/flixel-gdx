...
/**
 * <code>FlxEmitter</code> is a lightweight particle emitter. It can be used for
 * one-time explosions or for continuous fx like rain and fire.
 * <code>FlxEmitter</code> is not optimized or anything; all it does is launch
 * <code>FlxParticle</code> objects out at set intervals by setting their
 * positions and velocities accordingly. It is easy to use and relatively
 * efficient, relying on <code>FlxGroup</code>'s RECYCLE POWERS.
 * 
 * @author Ka Wing Chin
 */
public class FlxEmitter extends FlxGroup
{
	...
	public float y;
	/**
	 * The width of the emitter. Particles can be randomly generated from
	 * anywhere within this box.
	 */
	public float width;
	/**
	 * The height of the emitter. Particles can be randomly generated from
	 * anywhere within this box.
	 */
	public float height;
	/**
	...
	/**
	 * Determines whether the emitter is currently emitting particles. It is
	 * totally safe to directly toggle this.
	 */
	public boolean on;
	/**
	 * How often a particle is emitted (if emitter is started with Explode ==
	 * false).
	 */
	public float frequency;
	/**
	 * How long each particle lives once it is emitted. Set lifespan to 'zero'
	 * for particles to live forever.
	 */
	...
	/**
	 * Internal helper for the style of particle emission (all at once, or one
	 * at a time).
	 */
	...
	/**
	 * Internal variable for tracking the class to create when generating
	 * particles.
	 */
	protected Class<? extends FlxParticle> _particleClass;

	/**
	 * Creates a new <code>FlxEmitter</code> object at a specific position. Does
	 * NOT automatically generate or attach particles!
	 * 
	...
	/**
	 * This function generates a new array of particle sprites to attach to the
	 * emitter.
	 * 
	 * @param Graphics If you opted to not pre-configure an array of FlxParticle
	 *        objects, you can simply pass in a particle image or sprite sheet.
	 * @param Quantity The number of particles to generate when using the
	 *        "create from image" option.
	 * @param BakedRotations How many frames of baked rotation to use (boosts
	 *        performance). Set to zero to not use baked rotations.
	 * @param Multiple Whether the image in the Graphics param is a single
	 *        particle or a bunch of particles (if it's a bunch, they need to be
	 *        square!).
	 * @param Collide Whether the particles should be flagged as not 'dead'
	 *        (non-colliding particles are higher performance). 0 means no
	 *        collisions, 0-1 controls scale of particle's bounding box.
	 * 
	 * @return This FlxEmitter instance (nice for chaining stuff together, if
	 *         you're into that).
	 */
	...
	/**
	 * Called automatically by the game loop, decides when to launch particles
	 * and when to "die".
	 */
	@Override
	public void update()
	 ...
	 * @param Frequency Ignored if Explode is set to true. Frequency is how
	 *        often to emit a particle. 0 = never emit, 0.1 = 1 particle every
	 *        0.1 seconds, 5 = 1 particle every 5 seconds.
	...
	/**
	 * This function can be used both internally and externally to emit the next
	 * particle.
	 */
	...
	/**
	 * Set your own particle class type here. The custom class must extend
	 * <code>FlxParticle</code>. Default is <code>FlxParticle</code>.
	 */
	 ...
	 * 
	 * @param Width The desired width of the emitter (particles are spawned
	 *        randomly within these dimensions).
	...
	/**
	 * A more compact way of setting the angular velocity constraints of the
	 * emitter.
	 * 
	 * @param Min The minimum value for this range.
	 * @param Max The maximum value for this range.
