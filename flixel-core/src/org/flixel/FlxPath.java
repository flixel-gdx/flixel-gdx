...
/**
 * This is a simple path data container. Basically a list of points that a
 * <code>FlxObject</code> can follow. Also has code for drawing debug visuals.
 * <code>FlxTilemap.findPath()</code> returns a path object, but you can also
 * just make your own, using the <code>add()</code> functions below or by
 * creating your own array of points.
 * 
...
public class FlxPath
{
	 ...
	 * NOTE: does not affect world movement! Object scroll factors take care of
	 * that.
	 */
	public FlxPoint debugScrollFactor;
	/**
	 * Setting this to true will prevent the object from appearing when the
	 * visual debug mode in the debugger overlay is toggled on.
	 * 
	 * @default false
	 */
	 ...
	 * 
	 * @param Nodes Optional, can specify all the points for the path up front
	 *        if you want.
	 */
	...
	/**
	 * Add a new node to the path at the specified location and index within the
	 * path.
	 * 
	 * @param X X position of the new path point in world coordinates.
	 * @param Y Y position of the new path point in world coordinates.
	 * @param Index Where within the list of path nodes to insert this new
	 *        point.
	 */
	...
	/**
	 * Sometimes its easier or faster to just pass a point object instead of
	 * separate X and Y coordinates. This also gives you the option of not
	 * creating a new node but actually adding that specific
	 * <code>FlxPoint</code> object to the path. This allows you to do neat
	 * things, like dynamic paths.
	 * 
	 * @param Node The point in world coordinates you want to add to the path.
	 * @param AsReference Whether to add the point as a reference, or to create
	 *        a new point with the specified values.
	 */
	...
	/**
	 * Remove a node from the path. NOTE: only works with points added by
	 * reference or with references from <code>nodes</code> itself!
	 * 
	 * @param Node The point object you want to remove from the path.
	 * 
	 * @return The node that was excised. Returns null if the node was not
	 *         found.
	 */
	...
	/**
	 * Remove a node from the path using the specified position in the list of
	 * path nodes.
	 * 
	 * @param Index Where within the list of path nodes you want to remove a
	 *        node.
	 * 
	 * @return The node that was excised. Returns null if there were no nodes in
	 *         the path.
	 */
	...
	/**
	 * While this doesn't override <code>FlxBasic.drawDebug()</code>, the
	 * behavior is very similar. Based on this path data, it draws a simple
	 * lines-and-boxes representation of the path if the visual debug mode was
	 * toggled in the debugger overlay. You can use <code>debugColor</code> and
	 * <code>debugScrollFactor</code> to control the path's appearance.
	 * 
	...
	public void drawDebug(FlxCamera Camera)
	{
		...
		while(i < l)
		{
			...
			_point.x = node.x - (int) (Camera.scroll.x * debugScrollFactor.x); // copied
																				// from
																				// getScreenXY()
			...
			_point.x = nextNode.x - (int) (Camera.scroll.x * debugScrollFactor.x); // copied
																					// from
																					// getScreenXY()
