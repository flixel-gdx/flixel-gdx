 ...
 * convert arrays or images into strings that can be loaded.
 * 
...
public class FlxTilemap extends FlxObject
{
	...
	/**
	 * Better for levels with thick walls that look better with interior corner
	 * art.
	 */
	...
	/**
	 * Rendering helper, minimize new object instantiation on repetitive
	 * methods.
	 */
	...
	/**
	 * Internal reference to the bitmap data object that stores the original
	 * tile graphics.
	 */
	protected AtlasRegion _tiles;
	/**
	 * The actual Flash <code>BitmapData</code> object representing the current
	 * display state of the sprite.
	 */
	protected Sprite _framePixels;
	/**
	 * Internal list of buffers, one for each camera, used for drawing the
	 * tilemaps.
	 */
	protected Array<FlxTilemapBuffer> _buffers;
	/**
	 * Internal representation of the actual tile data, as a large 1D array of
	 * integers.
	 */
	protected IntArray _data;
	/**
	 * Internal representation of rectangles, one for each tile in the entire
	 * tilemap, used to speed up drawing.
	 */
	...
	/**
	 * Internal collection of tile objects, one for each type of tile in the map
	 * (NOTE one for every single tile in the whole map).
	 */
	protected Array<FlxTile> _tileObjects;

	/**
	 * Internal flag for checking to see if we need to refresh the tilemap
	 * display to show or hide the bounding boxes.
	 */
	protected boolean _lastVisualDebug;
	/**
	 * Internal, used to sort of insert blank tiles in front of the tiles in the
	 * provided graphic.
	 */
	 ...
	 *        if unspecified.
	 * @param AutoTile Whether to load the map using an automatic tile placement
	 *        algorithm. Setting this to either AUTO or ALT will override any
	 *        values you put for StartingIndex, DrawIndex, or CollideIndex.
	 * @param StartingIndex Used to sort of insert empty tiles in front of the
	 *        provided graphic. Default is 0, usually safest to leave it at
	 *        that. Ignored if AutoTile is set.
	 * @param DrawIndex Initializes all tile objects equal to and after this
	 *        index as visible. Default value is 1. Ignored if AutoTile is set.
	 * @param CollideIndex Initializes all tile objects equal to and after this
	 *        index as allowCollisions = ANY. Default value is 1. Ignored if
	 *        AutoTile is set. Can override and customize per-tile-type
	 *        collision behavior using <code>setTileProperties()</code>.
	 * 
	...
	public FlxTilemap loadMap(String MapData, String TileGraphic, int TileWidth, int TileHeight, int AutoTile, int StartingIndex, int DrawIndex, int CollideIndex)
	{
		...
		// create some tile objects that we'll use for overlap checks (one for
		// each tile)
		i = 0;
	...
	/**
	 * Main logic loop for tilemap is pretty simple, just checks to see if
	 * visual debug got turned on. If it did, the tilemap is flagged as dirty so
	 * it will be redrawn with debug info on the next draw call.
	 */
	@Override
	public void update()
	{
	 ...
	 * buffer. Called by draw().
	 * 
	 * @param Buffer The <code>FlxTilemapBuffer</code> you are rendering to.
	 * @param Camera The related <code>FlxCamera</code>, mainly for scroll
	 *        values.
	 */
	protected void drawTilemap(FlxTilemapBuffer Buffer, FlxCamera Camera)
	{
		...
		_point.x = (int) (Camera.scroll.x * scrollFactor.x) - x; // modified
																	// from
																	// getScreenXY()
		...
		_point.x = x - (int) (Camera.scroll.x * scrollFactor.x) + (screenXInTiles * _tileWidth); // copied
																									// from
																									// getScreenXY()
	...
	public void draw()
	{
		...
		if(!buffer.dirty)
		{
			_point.x = x - (int) (camera.scroll.x * scrollFactor.x) + buffer.x; // copied
																				// from
																				// getScreenXY()
		...
		_flashPoint.x = x - (int) (camera.scroll.x * scrollFactor.x) + buffer.x; // copied
																					// from
																					// getScreenXY()
	 ...
	 * 
	 * @param Simple If true, returns the data as copy, as a series of 1s and 0s
	 *        (useful for auto-tiling stuff). Default value is false, meaning it
	 *        will return the actual data array (NOT a copy).
	 * 
	 * @return An array the size of the tilemap full of integers indicating tile
	 *         placement.
	 */
	...
	/**
	 * Set the dirty flag on all the tilemap buffers. Basically forces a reset
	 * of the drawn tilemaps, even if it wasn'tile necessary.
	 * 
	...
	/**
	 * Find a path through the tilemap. Any tile with any collision flags set is
	 * treated as impassable. If no path is discovered then a null reference is
	 * returned.
	 * 
	 * @param Start The start point in world coordinates.
	 * @param End The end point in world coordinates.
	 * @param Simplify Whether to run a basic simplification algorithm over the
	 *        path data, removing extra points that are on the same line.
	 *        Default value is true.
	 * @param RaySimplify Whether to run an extra raycasting simplification
	 *        algorithm over the remaining path data. This can result in some
	 *        close corners being cut, and should be used with care if at all
	 *        (yet). Default value is false.
	 * 
	 * @return A <code>FlxPath</code> from the start to the end. If no path
	...
	public FlxPath findPath(FlxPoint Start, FlxPoint End, boolean Simplify, boolean RaySimplify)
	{
		...
		if(((_tileObjects.get(_data.get(startIndex))).allowCollisions > 0) || ((_tileObjects.get(_data.get(endIndex))).allowCollisions > 0))
			return null;
		...
		// finally load the remaining points into a new path object and return
		// it
	...
	/**
	 * Pathfinding helper function, strips out even more points by raycasting
	 * from one point to the next and dropping unnecessary points.
	 * 
	 * @param Points An array of <code>FlxPoint</code> nodes.
	 */
	...
	/**
	 * Pathfinding helper function, floods a grid with distance information
	 * until it finds the end point. NOTE: Currently this process does NOT use
	 * any kind of fancy heuristic! It's pretty brute.
	 * 
	 * @param StartIndex The starting tile's map index.
	 * @param EndIndex The ending tile's map index.
	 * 
	 * @return A libgdx <code>Array</code> of <code>FlxPoint</code> nodes. If
	 *         the end tile could not be found, then a null <code>Array</code>
	 *         is returned instead.
	 */
	...
	/**
	 * Pathfinding helper function, recursively walks the grid and finds a
	 * shortest path back to the start.
	 * 
	 * @param Data A libgdx <code>Array</code> of distance information.
	 * @param Start The tile we're on in our walk backward.
	 * @param Points A libgdx <code>Array</code> of <code>FlxPoint</code> nodes
	 *        composing the path from the start to the end, compiled in reverse
	 *        order.
	 */
	 ...
	 * <code>FlxObject</code> object in world space. If the group has a LOT of
	 * things in it, it might be faster to use <code>FlxG.overlaps()</code>.
	 * WARNING: Currently tilemaps do NOT support screen space overlap checks!
	 * 
	 * @param ObjectOrGroup The object being tested.
	 * @param InScreenSpace Whether to take scroll factors into account when
	 *        checking for overlap.
	 * @param Camera Specify which game camera you want. If null getScreenXY()
	 *        will just grab the first global camera.
	 * 
	 ...
	 * WARNING: Currently tilemaps do NOT support screen space overlap checks!
	 * 
	 * @param X The X position you want to check. Pretends this object (the
	 *        caller, not the parameter) is located here.
	 * @param Y The Y position you want to check. Pretends this object (the
	 *        caller, not the parameter) is located here.
	 * @param ObjectOrGroup The object or group being tested.
	 * @param InScreenSpace Whether to take scroll factors into account when
	 *        checking for overlap. Default is false, or
	 *        "only compare in world space."
	 * @param Camera Specify which game camera you want. If null getScreenXY()
	 *        will just grab the first global camera.
	 * 
	 * @return Whether or not the two objects overlap.
	...
	/**
	 * Checks if the Object overlaps any tiles with any collision flags set, and
	 * calls the specified callback function (if there is one). Also calls the
	 * tile's registered callback if the filter matches.
	 * 
	 * @param TargetObject The <code>FlxObject</code> you are checking for
	 *        overlaps against.
	 * @param Callback An optional function that takes the form
	 *        "myCallback(Object1:FlxObject,Object2:FlxObject)", where Object1
	 *        is a FlxTile object, and Object2 is the object passed in in the
	 *        first parameter of this method.
	 * @param FlipCallbackParams Used to preserve A-B list ordering from
	 *        FlxObject.separate() - returns the FlxTile object as the second
	 *        parameter instead.
	 * @param Position Optional, specify a custom position for the tilemap
	 *        (useful for overlapsAt()-type funcitonality).
	 * 
	 * @return Whether there were overlaps, or if a callback was specified,
	 *         whatever the return value of the callback was.
	 */
	public boolean overlapsWithCallback(FlxObject TargetObject, IFlxObject Callback, boolean FlipCallbackParams, FlxPoint Position)
	{
		...
		// Then loop through this selection of tiles and call
		// FlxObject.separate() accordingly
		...
		while(row < selectionHeight)
		{
			...
			while(column < selectionWidth)
			{
				...
				overlapFound = (TargetObject.x + TargetObject.width > tile.x) && (TargetObject.x < tile.x + tile.width) && (TargetObject.y + TargetObject.height > tile.y)
						&& (TargetObject.y < tile.y + tile.height);
	 ...
	 * tile's registered callback if the filter matches.
	 * 
	 * @param Object The <code>FlxObject</code> you are checking for overlaps
	 *        against.
	 * @param Callback An optional function that takes the form
	 *        "myCallback(Object1:FlxObject,Object2:FlxObject)", where Object1
	 *        is a FlxTile object, and Object2 is the object passed in in the
	 ...
	 * 
	 * @return An int containing the value of the tile at this spot in the
	 *         array.
	 */
	...
	/**
	 * Returns a new libgdx <code>Array</code> full of every map index of the
	 * requested tile type.
	 * 
	 * @param Index The requested tile type.
	 * 
	 * @return An <code>Array</code> with a list of all map indices of that tile
	 *         type.
	 */
	...
	/**
	 * Returns a new libgdx <code>Array</code> full of every coordinate of the
	 * requested tile type.
	 * 
	 * @param Index The requested tile type.
	 * @param Midpoint Whether to return the coordinates of the tile midpoint,
	 *        or upper left corner. Default is true, return midpoint.
	 * 
	 * @return An <code>Array</code> with a list of all the coordinates of that
	 *         tile type.
	 */
	...
	/**
	 * Adjust collision settings and/or bind a callback function to a range of
	 * tiles. This callback function, if present, is triggered by calls to
	 * overlap() or overlapsWithCallback().
	 * 
	 * @param Tile The tile or tiles you want to adjust.
	 * @param AllowCollisions Modify the tile or tiles to only allow collisions
	 *        from certain directions, use FlxObject constants NONE, ANY, LEFT,
	 *        RIGHT, etc. Default is "ANY".
	 * @param Callback The function to trigger, e.g.
	 *        <code>lavaCallback(Tile:FlxTile, Object:FlxObject)</code>.
	 * @param CallbackFilter If you only want the callback to go off for certain
	 *        classes or objects based on a certain class, set that class here.
	 ...
	 *        hiding the edges.
	 * @param UpdateWorld Whether to update the collision system's world size,
	 *        default value is true.
	 */
	...
	/**
	 * Get the world coordinates and size of the entire tilemap as a
	 * <code>FlxRect</code>.
	 * 
	 * @param Bounds Optional, pass in a pre-existing <code>FlxRect</code> to
	 *        prevent instantiation of a new object.
	 * 
	 * @return A <code>FlxRect</code> containing the world coordinates and size
	 *         of the entire tilemap.
	 */
	 ...
	 * through a tile, it stores that point and returns false.
	 * 
	 ...
	 *        means more checks!
	 * @return Returns true if the ray made it from Start to End without hitting
	 *         anything. Returns false and fills Result if a tile was hit.
	 */
	...
	/**
	 * Converts a <code>Pixmap</code> object to a comma-separated string. Black
	 * pixels are flagged as 'solid' by default, non-black pixels are set as
	 * non-colliding. Black pixels must be PURE BLACK.
	 * 
	 * @param Graphic A libgdx <code>Pixmap</code> object, preferably black and
	 *        white.
	 ...
	 * @param ColorMap An array of color values (int 0xAARRGGBB) in the order
	 *        they're intended to be assigned as indices
	 * 
	 * @return A comma-separated string containing the level data in a
	 *         <code>FlxTilemap</code>-friendly format.
	 */
	 ...
	 * 
	 * @param Bitmap A Flash <code>BitmapData</code> object, preferably black
	 *        and white.
	 * @param Invert Load white pixels as solid instead.
	 * @param Scale Default is 1. Scale of 2 means each pixel forms a 2x2 block
	 *        of tiles, and so on.
