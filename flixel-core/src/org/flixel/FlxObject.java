 ...
 * <code>FlxSprite</code>, <code>FlxText</code>, etc). It includes some basic
 * attributes about game objects, including retro-style flickering, basic state
 * information, sizes, scrolling, and basic physics and motion.
 * 
 * @author Ka Wing Chin
...
public class FlxObject extends FlxBasic
{
	...
	/**
	 * Special-case const meaning no collisions, used mainly by
	 * <code>allowCollisions</code> and <code>touching</code>.
	 */
	static public final int NONE = 0;
	/**
	 * Special-case const meaning up, used mainly by
	 * <code>allowCollisions</code> and <code>touching</code>.
	 */
	static public final int CEILING = UP;
	/**
	 * Special-case const meaning down, used mainly by
	 * <code>allowCollisions</code> and <code>touching</code>.
	 */
	static public final int FLOOR = DOWN;
	/**
	 * Special-case const meaning only the left and right sides, used mainly by
	 * <code>allowCollisions</code> and <code>touching</code>.
	 */
	static public final int WALL = LEFT | RIGHT;
	/**
	 * Special-case const meaning any direction, used mainly by
	 * <code>allowCollisions</code> and <code>touching</code>.
	 */
	static public final int ANY = LEFT | RIGHT | UP | DOWN;

	/**
	 * Handy const used during collision resolution (see
	 * <code>separateX()</code> and <code>separateY()</code>).
	 */
	...
	/**
	 * Path behavior controls: move from the start of the path to the end then
	 * turn around and go back to the start, over and over.
	 */
	static public final int PATH_YOYO = 0x001000;
	/**
	 * Path behavior controls: ignores any vertical component to the path data,
	 * only follows side to side.
	 */
	static public final int PATH_HORIZONTAL_ONLY = 0x010000;
	/**
	 * Path behavior controls: ignores any horizontal component to the path
	 * data, only follows up and down.
	 */
	...
	/**
	 * The virtual mass of the object. Default value is 1. Currently only used
	 * with <code>elasticity</code> during collision resolution. Change at your
	 * own risk; effects seem crazy unpredictable so far!
	 */
	public float mass;
	/**
	 * The bounciness of this object. Only affects collisions. Default value is
	 * 0, or "not bouncy at all."
	 */
	public float elasticity;
	/**
	 * How fast the speed of this object is changing. Useful for smooth movement
	 * and gravity.
	 */
	...
	/**
	 * Set the angle of a sprite to rotate it. WARNING: rotating sprites
	...
	/**
	 * Use in conjunction with <code>angularAcceleration</code> for fluid spin
	 * speed control.
	 */
	 ...
	 * that governs how much this object is affected by the camera subsystem. 0
	 * means it never moves, like a HUD element or far background graphic. 1
	 * means it scrolls along a the same speed as the foreground layer.
	...
	/**
	 * This is just a pre-allocated rectangle container to be used however you
	 * like
	 */
	 ...
	 * (see <code>updateMotion()</code>). FlxObject, FlxSprite and FlxText
	 * default to true. FlxTileblock and FlxTilemap default to false.
	 */
	public boolean moves;
	/**
	 * Bit field of flags (use with UP, DOWN, LEFT, RIGHT, etc) indicating
	 * surface contacts. Use bitwise operators to check the values stored here,
	 * or use touching(), justStartedTouching(), etc. You can even use them
	 * broadly as boolean values if you're feeling saucy!
	 */
	public int touching;
	/**
	 * Bit field of flags (use with UP, DOWN, LEFT, RIGHT, etc) indicating
	 * surface contacts from the previous game loop step. Use bitwise operators
	 * to check the values stored here, or use touching(),
	 * justStartedTouching(), etc. You can even use them broadly as boolean
	 * values if you're feeling saucy!
	 */
	public int wasTouching;
	/**
	 * Bit field of flags (use with UP, DOWN, LEFT, RIGHT, etc) indicating
	 * collision directions. Use bitwise operators to check the values stored
	 * here. Useful for things like one-way platforms (e.g. allowCollisions =
	...
	public FlxPath path;
	/**
	 * The speed at which the object is moving on the path. When an object
	 * completes a non-looping path circuit, the pathSpeed will be zeroed out,
	 * but the <code>path</code> reference will NOT be nulled out. So
	...
	/**
	 * Internal helper, tracks which node of the path this object is moving
	 * toward.
	 */
	...
	/**
	 * Internal flag for whether the object's angle should be adjusted to the
	 * path angle during path follow behavior.
	 */
	 ...
	 * the game loop. In <code>FlxObject</code> this function handles
	 * integrating the objects motion based on the velocity and acceleration
	 * settings, and tracking/clearing the <code>touching</code> flags.
	 */
	@Override
	 ...
	 * Useful for cases when you need to update this but are buried down in too
	 * many supers. Does a slightly fancier-than-normal integration to help with
	 * higher fidelity framerate-independenct motion.
	 */
	...
	/**
	 * Rarely called, and in this case just increments the visible objects count
	 * and calls <code>drawDebug()</code> if necessary.
	 */
	@Override
	public void draw()
	{
	...
	public void drawDebug(FlxCamera Camera)
	{
		...
		float boundingBoxX = x - (int) (Camera.scroll.x * scrollFactor.x); // copied
																			// from
																			// getScreenXY()
	...
	/**
	 * Call this function to give this object a path to follow. If the path does
	 * not have at least one node in it, this function will log a warning
	 * message and return.
	 * 
	 ...
	 * @param Mode Optional, controls the behavior of the object following the
	 *        path using the path behavior constants. Can use multiple flags at
	 *        once, for example PATH_YOYO|PATH_HORIZONTAL_ONLY will make an
	 *        object move back and forth along the X axis of the path only.
	 * @param AutoRotate Automatically point the object toward the next node.
	 *        Assumes the graphic is pointing upward. Default behavior is false,
	 *        or no automatic rotation.
	 */
	 ...
	 * 
	 * @param DestroyPath Tells this function whether to call destroy on the
	 *        path object. Default value is false.
	 */
	 ...
	 * half of the function decides if the object can advance to the next node
	 * in the path, while the second half handles actually picking a velocity
	...
	/**
	 * Checks to see if some <code>FlxObject</code> or <code>FlxGroup</code>
	 * overlaps this <code>FlxObject</code>. If the group has a LOT of things in
	 * it, it might be faster to use <code>FlxG.overlaps()</code>. WARNING:
	 * Currently tilemaps do NOT support screen space overlap checks!
	 * 
	...
	public boolean overlaps(FlxBasic ObjectOrGroup, boolean InScreenSpace, FlxCamera Camera)
	{
		...
		{
			// Since tilemap's have to be the caller, not the target, to do
			// proper tile-based collisions,
		...
		return (objectScreenPos.x + object.width > _point.x) && (objectScreenPos.x < _point.x + width) && (objectScreenPos.y + object.height > _point.y)
	...
	public boolean overlapsAt(float X, float Y, FlxBasic ObjectOrGroup, boolean InScreenSpace, FlxCamera Camera)
	{
		...
		if(Camera == null)
			Camera = FlxG.camera;
		FlxPoint objectScreenPos = object.getScreenXY(null, Camera);
		_point.x = X - (Camera.scroll.x * scrollFactor.x); // copied from
															// getScreenXY()
	 ...
	 * 
	 * @param Point Takes a <code>FlxPoint</code> object and assigns the
	...
	/**
	 * Tells this object to flicker, retro-style. Pass a negative value to
	 * flicker forever.
	 * 
	 ...
	 * the object will collide from, use collision constants (like LEFT, FLOOR,
	 ...
	 * 
	 * @param Point Allows you to pass in an existing <code>FlxPoint</code>
	...
	/**
	 * Handy function for reviving game objects. Resets their existence flags
	 * and position.
	 * 
	...
	/**
	 * Handy function for checking if this object is touching a particular
	 * surface. For slightly better performance you can just &amp; the value
	 * directly into <code>touching</code>. However, this method is good for
	 * readability and accessibility.
	 * 
	 * @param Direction Any of the collision flags (e.g. LEFT, FLOOR, etc).
	 * 
	 * @return Whether the object is touching an object in (any of) the
	 *         specified direction(s) this frame.
	 */
	 ...
	 * @param Direction Any of the collision flags (e.g. LEFT, FLOOR, etc).
	 * 
	 * @return Whether the object just landed on (any of) the specified
	 *         surface(s) this frame.
	 */
	 ...
	 * 
	 * @param Damage How much health to take away (use a negative number to give
	 *        a health bonus).
	 */
	 ...
	 * @param Object2 Any other <code>FlxObject</code>.
	 * 
	 * @return Whether the objects in fact touched and were separated along the
	 *         X axis.
	 */
	static public boolean separateX(FlxObject Object1, FlxObject Object2)
	{
		...
		if(obj1delta != obj2delta)
		{
			...
			if((obj1x + obj1width > obj2x) && (obj1x < obj2x + obj2width) && (obj1y + obj1height > obj2y) && (obj1y < obj2y + obj2height))
			{
				...
				// If they did overlap (and can), figure out by how much and
		...
		// Then adjust their positions and velocities accordingly (if there was
		// any overlap)
		if(overlap != 0)
		{
