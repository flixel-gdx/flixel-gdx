...
/**
 * The main "game object" class, the sprite is a <code>FlxObject</code> with a
 * bunch of graphics options and abilities, like animation and stamping.
 * 
 * @author Ka Wing Chin
...
public class FlxSprite extends FlxObject
{
	...
	/**
	 * WARNING: The origin of the sprite will default to its center. If you
	 * change this, the visuals and the collisions will likely be pretty
	 * out-of-sync if you do any rotation.
	 */
	public FlxPoint origin;
	/**
	 * If you changed the size of your sprite object after loading or making the
	 * graphic, you might need to offset the graphic away from the bound box to
	 * center it the way you want.
	 */
	public FlxPoint offset;

	/**
	 * Change the size of your sprite's graphic. NOTE: Scale doesn't currently
	 * affect collisions automatically, you will need to adjust the width,
	 * height and offset manually.
	 */
	public FlxPoint scale;
	/**
	 * Blending modes, just like Photoshop or whatever. E.g. "multiply",
	 * "screen", etc.
	 * 
	 * @default null
	 */
	public String blend;
	/**
	 * GLES20 Blending modes, just like Photoshop or whatever. Use
	 * <code>BlendModeGL20.blend()</code> to create a blend. NOTE: Requires
	 * GLES20.
	 * 
	 * @default null
	 */
	...
	/**
	 * The sprite that will be blended with the base. Only used with
	 * <code>blendGL20</code>. NOTE: Requires GLES20.
	 * 
	 * @default null
	 */
	...
	/**
	 * Controls whether the object is smoothed when rotated, affects
	 * performance.
	 * 
	 * @default false
	 */
	...
	/**
	 * The width of the actual graphic or image being displayed (not necessarily
	 * the game object/bounding box). NOTE: Edit at your own risk!! This is
	 * intended to be read-only.
	 */
	public int frameWidth;
	/**
	 * The height of the actual graphic or image being displayed (not
	 * necessarily the game object/bounding box). NOTE: Edit at your own risk!!
	 * This is intended to be read-only.
	 */
	public int frameHeight;
	/**
	 * The <code>Sprite</code> object representing the current
	 * display state of the sprite.
	 */
	public Sprite framePixels;
	/**
	 * Set this flag to true to force the sprite to update during the draw()
	 * call. NOTE: Rarely if ever necessary, most sprite operations will flip
	...
	/**
	 * Internal, keeps track of whether the sprite was loaded with support for
	 * automatic reverse/mirroring.
	 */
	...
	/**
	 * Internal, keeps track of the current frame of animation. This is NOT an
	 * index into the tile sheet, but the frame number in the animation object.
	 */
	protected int _curFrame;
	/**
	 * Internal, keeps track of the current index into the tile sheet based on
	 * animation or rotation.
	 */
	protected int _curIndex;
	/**
	 * Internal, tracker for the maximum number of frames that can fit on the
	 * tile sheet, used with read-only getter. WARNING: assumes each row in the
	 * sprite sheet is full!
	 */
	...
	/**
	 * Internal tracker for the animation callback. Default is null. If
	 * assigned, will be called each time the current frame changes. A function
	 * that has 3 parameters: a string name, an int frame number, and an int
	 * frame index.
	 */
	protected IFlxAnim _callback;
	/**
	 * Internal tracker for what direction the sprite is currently facing, used
	 * with Flash getter/setter.
	 */
	...
	/**
	 * Internal tracker for how many frames of "baked" rotation there are (if
	 * any).
	 */
	protected float _bakedRotation;
	/**
	 * Internal, stores the entire source graphic (not the current displayed
	 * animation frame), used with getter/setter.
	 */
	protected AtlasRegion _pixels;
	/**
	 * Internal tracker for reloading the texture if its pixmap has been
	 * modified.
	 */
	protected ManagedTextureData _newTextureData;

	/**
	 * Creates a white 8x8 square <code>FlxSprite</code> at the specified
	 * position. Optionally can load a simple, one-frame graphic instead.
	 * 
	 * @param X The initial X position of the sprite.
	 * @param Y The initial Y position of the sprite.
	 * @param SimpleGraphic The graphic you want to display (OPTIONAL - for
	 *        simple stuff only, do NOT use for animated images!).
	 */
	 ...
	 * @param Animated Whether the Graphic parameter is a single sprite or a row
	 *        of sprites.
	 * @param Reverse Whether you need this class to generate horizontally
	 *        flipped versions of the animation frames.
	 * @param Width Optional, specify the width of your sprite (helps FlxSprite
	 *        figure out what to do with non-square sprites or sprite sheets).
	 * @param Height Optional, specify the height of your sprite (helps
	 *        FlxSprite figure out what to do with non-square sprites or sprite
	 *        sheets).
	 * @param Unique Optional, whether the graphic should be a unique instance
	 *        in the graphics cache. Default is false.
	 * 
	 * @return This FlxSprite instance (nice for chaining stuff together, if
	 *         you're into that).
	 */
	...
	/**
	 * Create a pre-rotated sprite sheet from a simple sprite. This can make a
	 * huge difference in graphical performance!
	 * 
	 * @param Graphic The image you want to rotate and stamp.
	 * @param Rotations The number of rotation frames the final sprite should
	 *        have. For small sprites this can be quite a large number (360
	 *        even) without any problems.
	 * @param Frame If the Graphic has a single row of square animation frames
	 *        on it, you can specify which of the frames you want to use here.
	 *        Default is -1, or "use whole graphic."
	 * @param AntiAliasing Whether to use high quality rotations when creating
	 *        the graphic. Default is false.
	 * @param AutoBuffer Whether to automatically increase the image size to
	 *        accommodate rotated corners. Default is false. Will create frames
	 *        that are 150% larger on each axis than the original frame or
	 *        graphic.
	 * 
	 * @return This FlxSprite instance (nice for chaining stuff together, if
	 *         you're into that).
	 ...
	 * @param Unique Whether the graphic should be a unique instance in the
	 *        graphics cache. Default is false.
	 * @param Key Optional parameter - specify a string key to identify this
	 *        graphic in the cache. Trumps Unique flag.
	 * 
	 * @return This FlxSprite instance (nice for chaining stuff together, if
	 *         you're into that).
	 */
	...
	/**
	 * Automatically called after update() by the game loop, this function just
	...
	/**
	 * Called by game loop, updates then blits or renders current frame of
	 ...
	 * 
	 * @param Brush The image you want to use as a brush or stamp or pen or
	 *        whatever.
	...
	public void stamp(FlxSprite Brush, int X, int Y)
	{
		...
		stamp(brushPixmap, Brush.framePixels.getRegionX(), Brush.framePixels.getRegionY() - Brush.frameHeight, Brush.frameWidth, Brush.frameHeight, X + _pixels.getRegionX(), Y
				+ _pixels.getRegionY());
	 ...
	 * @param DestinationX The X coordinate of the brush's top left corner on
	 *        this sprite.
	 * @param DestinationY The Y coordinate of the brush's top right corner on
	 *        this sprite.
	 */
	...
	/**
	 * This function draws a line on this sprite from position X1,Y1 to position
	 * X2,Y2 with the specified color.
	 * 
	...
	/**
	 * Internal function for updating the sprite's animation. Useful for cases
	 * when you need to update this but are buried down in too many supers. This
	 * function is called automatically by <code>FlxSprite.postUpdate()</code>.
	 */
	 ...
	 * 
	 * @param Force Force the frame to redraw, even if its not flagged as
	 *        necessary.
	 */
	 ...
	 * 
	 * @param AnimationCallback A function that has 3 parameters: a string name,
	 *        an int frame number, and an int frame index.
	 */
	...
	/**
	 * Plays an existing animation (e.g. "run"). If you call an animation that
	 * is already playing it will be ignored.
	 * 
	...
	/**
	 * Tell the sprite to change to a random frame of animation Useful for
	 * instantiating particles or other weird things.
	 */
	...
	/**
	 * Helper function that adjusts the offset automatically to center the
	 * bounding box within the graphic.
	 * 
	 * @param AdjustPosition Adjusts the actual X and Y position just once to
	 *        match the offset change. Default is false.
	 */
	 ...
	 * 
	 * @param Color The color that needs to be replaced.
	 * @param NewColor The new color that will replaced the old one.
	 * @param FetchPositions Whether the positions where the color is replaced
	 *        should be saved or not.
	 * @return An array of <code>FlxPoint</code>s that points where the color is
	 *         replaced.
	 */
	...
	/**
	 * Set <code>facing</code> using <code>FlxSprite.LEFT</code>,
	 * <code>RIGHT</code>, <code>UP</code>, and <code>DOWN</code> to take
	 * advantage of flipped sprites and/or just track player orientation more
	 * easily.
	 */
	 ...
	 * <code>color</code> IGNORES ALPHA. To change the opacity use
	 * <code>alpha</code>. Tints the whole sprite to be this color (similar to
	...
	public void setFrame(int Frame)
	{
		...
		{
			// FlxG.log("WARNING: The frame number of a " + this +
			// " be less than its `numFrames` value.");
	 ...
	 * 
	 * @param NumFrames The number of frames on the sprite sheet. Has to be a
	 *        value between <code>1</code> and <code>maxFrames</code>.
	 */
	 ...
	 * <code>FlxObject</code>'s implementation in that it takes the actual
	 * graphic into account, not just the hitbox or bounding box or whatever.
	 * 
	 ...
	 * <code>FlxSprite</code> object's current displayed pixels. This check is
	 * ALWAYS made in screen space, and always takes scroll factors into
	 * account.
	 * 
