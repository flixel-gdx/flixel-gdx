...
/**
 * This class contains the record, stop, play, and step 1 frame buttons seen on
 * the top edge of the debugger overlay.
 * 
...
public class VCR
{
	...
	// static protected final FILE_TYPES:Array = [new
	// FileFilter("Flixel Game Recording", "*.fgr")];
	...
	/**
	 * Creates the "VCR" control panel for debugger pausing, stepping, and
	 * recording.
	 */
	public VCR()
	{
		...
		// _runtimeDisplay.defaultTextFormat = new
		// TextFormat("Courier",12,0xffffff,null,null,null,null,null,"center");
	...
	/**
	 * Usually called by FlxGame when a requested recording has begun. Just
	...
	/**
	 * Usually called by FlxGame when a replay has been stopped. Just updates
	 * the VCR GUI so the buttons are in the right state.
	 */
	public void stopped()
	{
		// _stop.visible = false;
	...
	/**
	 * Usually called by FlxGame when a requested replay has begun. Just updates
	 * the VCR GUI so the buttons are in the right state.
	 */
	public void playing()
	{
	...
	/**
	 * Called when the "open file" button is pressed. Opens the file dialog and
	 * registers event handlers for the file dialog.
	 */
	public void onOpen()
	{
	...
	/**
	 * Called when a file is picked from the file dialog. Attempts to load the
	 * file and registers file loading event handlers.
	 * 
	 * @param E Flash event.
	 */
	...
	/**
	 * Called when a file is opened successfully. If there's stuff inside, then
	 * the contents are loaded into a new replay.
	 * 
	 * @param E Flash Event.
	 */
	...
	/**
	 * Called when the user presses the white record button. If Alt is pressed,
	 * the current state is reset, and a new recording is requested. If Alt is
	 * NOT pressed, the game is reset, and a new recording is requested.
	 * 
	 * @param StandardMode Whether to reset the whole game, or just this
	 *        <code>FlxState</code>. StandardMode == false is useful for
	 *        recording demos or attract modes.
	 */
	...
	/**
	 * Called when the user presses the red record button. Stops the current
	 * recording, opens the save file dialog, and registers event handlers.
	 */
	...
	/**
	 * Called when the user presses the Rewind-looking button. If Alt is
	 * pressed, the entire game is reset. If Alt is NOT pressed, only the
	 * current state is reset. The GUI is updated accordingly.
	 * 
	 * @param StandardMode Whether to reset the current game (== true), or just
	 *        the current state. Just resetting the current state can be very
	 *        handy for debugging.
	 */
	...
	/**
	 * Called when the user presses the Pause button. This is different from
	 * user-defined pause behavior, or focus lost behavior. Does NOT pause music
	 * playback!!
	 */
	public void onPause()
	{
	...
	/**
	 * Called when the user presses the Play button. This is different from
	 * user-defined unpause behavior, or focus gained behavior.
	 */
	public void onPlay()
	{
		paused = false;
	...
	/**
	 * Called when the user presses the fast-forward-looking button. Requests a
	 * 1-frame step forward in the game loop.
	 */
	public void onStep()
	{
		if(!paused)
	...
	/**
	 * If the mouse is pressed down, check to see if the user started pressing
	 * down a specific button.
	 * 
	 * @param E Flash mouse event.
	 */
	...
	/**
	 * If the mouse is released, check to see if it was released over a button
	 * that was pressed. If it was, take the appropriate action based on button
	...
	/**
	 * This function checks to see what button the mouse is currently over. Has
	 * some special behavior based on whether a recording is happening or not.
	 * 
	...
	/**
	 * Figures out what buttons to highlight based on the _overWhatever and
	 * _pressingWhatever variables.
	 */
